...
| TC-11 | Per-assignment structured fields extracted: title, due date, point value, pinned against the real capture in `snapshots/`. Assignment descriptions are ignored in this proof of concept (decision D8). Record scope is the To-Do sidebar only (decision D9): a record is one `.ToDoSidebarItem` whose title comes from `aria-label="Assignment, <Title>"`; due date and points come from the `<li>` texts of `[data-testid=ToDoSidebarItem__InformationRow]`. Activity-stream rows are never parsed into records. (spec.md:16; 49-50; decisions D1/D7/D8/D9) | All three fields extracted for every assignment the capture renders as a To-Do sidebar item, using those confirmed selectors; records with no shown due date/points handled without error; stream-only titles contribute no records. | Fields extracted for only a subset of the To-Do items a capture shows. | A required field is never extracted from any To-Do item, or parsing relies on guessed, unconfirmed selectors. | Yes | Parse `snapshots/Canvas_*.html`; assert exactly the 3 To-Do titles (Lab: OpenCode Studio, Homework 3, Writing as Thinking - Week 5) and their due date/point values; assert no stream-derived records. |
...
- **D9 — Reject the activity-stream fallback (2026-09-24; supersedes the
  stream-row clause in D1 and Q2 of CURRENT_TASK.md).** The parser is
  To-Do-only. Rationale (charter value #1, Correctness over speed): an
  "Assignment Due Date Changed" stream row carries the *change timestamp*, not
  the assignment's due date or points, so a stream-derived record is always
  `{due: not shown, points: not shown}` and can never produce a correct
  positive change. Its only possible outputs are a no-op (row on both sides),
  a false `Assignment removed` (the stream is a rolling window, so rows age out
  even when the assignment exists), or a mislabeled `Assignment added` (a fresh
  stream row is evidence of a due-date *change*, not an addition). TC-11 is
  reworded to To-Do scope; `rubric.json` records D9.
